...
const fetch = require("node-fetch"); // Node.js 18 이상에서는 내장 fetch 사용 가능
...
const renderAboutPage = async (req, res) => {
  try {
    ...
    if (data.success) {
      ...
      // 가능한 모든 카테고리 목록 추출 (예: tags 또는 별도의 'category' 필드 사용)
      ...
      res.render("about", {
        ...
        availableCategories: availableCategories, // 추가된 카테고리 목록
        selectedConvenience: selectedConvenience,
        selectedEvent: selectedEvent,
        selectedCategory: selectedCategory, // 선택된 카테고리
        sortBy: sortBy,
        userId: userId,
      });
    } else {
